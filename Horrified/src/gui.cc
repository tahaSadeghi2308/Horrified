...
void Gui::run() {
    ...
    while (!WindowShouldClose()) {
        ...
        if(pageNumber != PageNumbers::MONSTERPHASE_PAGE)
        {
            ...
            {
                static bool newCard {true};
                static MonsterCard CurrentMonsterCard;

                if(newCard)
                {
                    CurrentMonsterCard  = sys->getRandomMonstCard();
                    for (auto monst : sys->getAllMonsters()){
                        if (monst != nullptr) {
                            monst->putItem(CurrentMonsterCard.itemCount);
                            monst->doEvent(CurrentMonsterCard.name , currentHero);
                            break;
                        }
                    }
                    newCard = false;
                    ...
                    monsterPhasePage(currentHero , CurrentMonsterCard  );
