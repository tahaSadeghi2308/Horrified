...
void HeroBase::moveAction()
{
    std::cout << "where do you want to go (select a number)\n";
            ...
            for (auto& vill : getPlace()->getVillage())
            {
                ...
                targetPlaces[choice - 1]->addVill(vill);
                vill->changeLoc(targetPlaces[choice - 1]);
    
            }

            getPlace()->deleteHero(getHeroName());
            targetPlaces[choice-1]->addHero(std::make_shared<HeroBase>(*this));// we have some problem
...
void HeroBase::addItem(Item target)
{
    heroItems.push_back(target);
}
...
{
    std::vector<std::shared_ptr<Place>> neigh = getPlace()->getNeighbors();
...
void HeroBase::guideAction()
{
    std::vector<std::shared_ptr<Villager>> temp = loc->getVillage(); 
        ...
        std::vector<std::shared_ptr<Place>> neighbors = loc->getNeighbors();
        ...
        shared_ptr<Place> chosenPlace = neighbors[choose-1];
...
void HeroBase::pickUpAction()
{
    std::vector<Item> available = loc->getItems();
        ...
        heroItems.push_back(available[choice-1]);
